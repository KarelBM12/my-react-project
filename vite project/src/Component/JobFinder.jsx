...
const JobFinder = () => {
  ...
  useEffect(() => {
    const fetchJobdata = async () => {
      try {
        const response = await axios.get("http://localhost:9300/api/formdata");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formId) {
        await axios.put(`http://localhost:9300/api/formdata/${formId}`, formData);
        console.log("Form Data Updated:", formData);
      } else {
        const response = await axios.post("http://localhost:9300/api/formdata", formData);
